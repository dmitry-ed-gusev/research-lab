...
public class DbfEmailsListBuilder {
    ...
    public TreeMap<String, Integer> getEmailsList(Defaults.RecipientType recipientType) throws IllegalAccessException, InstantiationException, ClassNotFoundException {
        ...
        if (!StringUtils.isBlank(firmDbPath) && !StringUtils.isBlank(fleetDbPath)) {
            ...
            if (fleetDBFile.exists() && fleetDBFile.isDirectory() && firmDBFile.exists() && firmDBFile.isDirectory()) {
                ...
                try {
                    fleetConn = DBUtilities.getDBConn(fleetConfig);
                    ...
                    if (fleetRs.next()) {
                        ...
                        firmConn = DBUtilities.getDBConn(firmConfig);
                        ...
                        do {
                            ...
                            if (operatorId > 0) {
                                ...
                                if (firmRs.next()) {
                                    operatorEmail = firmRs.getString("email");
                                } else {
                                    operatorEmail = null;
                                }
                            } else {
                            ...
                            if (shipownerId > 0) {
                                ...
                                firmRs = firmStmt.executeQuery();
                                if (firmRs.next()) {
                                    shipownerEmail = firmRs.getString("email");
                                } else {
                                    shipownerEmail = null;
                                }
                            } else {
                            ...
                            if (ownerId > 0) {
                                ...
                                firmRs = firmStmt.executeQuery();
                                if (firmRs.next()) {
                                    ownerEmail = firmRs.getString("email");
                                } else {
                                    ownerEmail = null;
                                }
                            } else {
                    ...
                    // Если судов воопче не найдено - сообщаем в лог
                        ...
                        logger.warn("No ships! Can't process.");
                    }
        ...
        // Если пути пусты - сообщаем в лог об ошибке и возвращаем NULL
            ...
            logger.error("DB FLEET path [" + fleetDbPath + "] or DB FIRMS path [" + firmDbPath + "] is empty! Can't process!");
        }
