    ...
    public static long getChecksum(String fileName) {
        ...
        try {
            ...
            // Сообщение о пустом имени файла
                ...
                logger.warn("Received file name is EMPTY!");
            }
        }
        // Перехватываем ИС
        catch (FileNotFoundException e) {
            logger.error("ERROR occured while CRC32 calculating: " + e.getMessage());
        } catch (IOException e) {
            logger.error("ERROR occured while CRC32 calculating: " + e.getMessage());
        }
        // Обязательно необходимо закрыть за собой потоки
            ...
            try {
                ...
                logger.error("Can't close stream for file [" + fileName + "]! Reason: [" + e.getMessage() + "].");
            }
        }
        return result;
    }
