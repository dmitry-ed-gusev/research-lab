...
public final class DBasesLoaderCore {
            ...
            if (!StringUtils.isBlank(tableName)) {
                ...
                if (!StringUtils.isBlank(path) && new File(path).exists() && new File(path).isDirectory()) {
                    ...
                    if (lowerTimestamp != null) {
                        ...
                        logger.debug("There is no lower timestamp for table [" + tableName + "].");
                    }
                    ...
                    if (upperTimestamp != null) {
                        ...
                        if (((keysList != null) && (!keysList.isEmpty())) || (lowerTimestamp != null)) {
                            sql.append(" and ");
                        } else {
                            sql.append(" where ");
                        }
                        ...
                        logger.debug("There is no upper timestamp for table [" + tableName + "].");
                    }
                    ...
                    try {
                        ...
                        if (lowerTimestamp != null) {
                            ...
                            if (upperTimestamp != null) {
                                stmt.setTimestamp(2, upperTimestamp);
                            }
                        } else if (upperTimestamp != null) {
                            stmt.setTimestamp(1, upperTimestamp);
                        }
                        ...
                        do {
                            ...
                            if ((srs != null) && !srs.isEmpty()) {
                                ...
                                if (descriptor == null) {
                                    descriptor = new Descriptor(tableName, DBasesLoaderCore.ObjectType.TABLE);
                                }
                        ...
                        if (descriptor != null) {
                            MyIOUtils.serializeObject(descriptor, path, DESCRIPTOR_FILE_NAME, EXTENSION_TABLE, false);
                        }
                        // Если дескриптор не инициализирован, то просто сообщим в лог, что ничего не выгружено
                        else {
                            logger.info("No data was serialized for table [" + tableName + "].");
                        }
                    }
                    ...
                    // Освобождение ресурсов
                        ...
                        try {
                            ...
                            logger.error("Can't free resources! Message: [" + e.getMessage() + "].");
                        }
                    }
                } else {
                    throw new JdbException("Path [" + path + "] is empty, not exists or not a directory!");
                }
            } else {
                throw new JdbException("Empty table name [" + tableName + "]!");
            }
        } else {
            throw new JdbException("Empty db connection!");
        }
    ...
    protected static boolean unloadDBToDisk(Connection conn, String path, String dbName, ArrayList<String> tablesList,
        ...
        if (conn != null) {
            ...
            if (!StringUtils.isBlank(dbName) && (tablesList != null) && (!tablesList.isEmpty())) {
                ...
                if (!StringUtils.isBlank(path) && (new File(path).exists()) && (new File(path).isDirectory())) {
                    ...
                    for (String tableName : tablesList) {
                        if (!StringUtils.isBlank(tableName)) {
                            ...
                            if (new File(MyIOUtils.fixFPath(path, true) + tableName.toUpperCase()).mkdir()) {
                                ...
                                if (timedModel != null) {
                                    timestamp = timedModel.getTimestampForTable(tableName);
                                }
                                // Получаем список ключей для выгрузки из данной таблицы
                                ArrayList<Integer> keysList = null;
                                if (integrityModel != null) {
                                    keysList = integrityModel.getKeysListForTable(tableName);
                                }
                                ...
                                try {
                                    ...
                                    if (unloadTableResult) {
                                        if (descriptor == null) {
                                            descriptor = new Descriptor(dbName, DBasesLoaderCore.ObjectType.DATABASE);
                                        }
                                ...
                                logger.error("Can't create catalog [" + (MyIOUtils.fixFPath(path, true) + tableName) + "] for " +
                                        "table [" + tableName + "]! Table skipped.");
                            }
                        }
                        // Пустое имя таблицы - сообщим в лог (WARN)
                        else {
                            logger.warn("Empty table name! Programmer error?");
                        }
                    ...
                    if (descriptor != null) {
                        ...
                        try {
                            MyIOUtils.serializeObject(descriptor, path, DESCRIPTOR_FILE_NAME, EXTENSION_DB, false);
                        } catch (IOException e) {
                            throw new JdbException("Can't serialize db descriptor!", e);
                        }
                    }
                    // Если дескриптор проинициализирован не был - это означает, что никакие данные вообще не были выгружены.
                    // Сообщим об этом в лог.
                    else {
                        logger.info("No data was serialized for DB [" + dbName + "].");
                    }
                }
            ...
            // Ошибочное имя БД или пустой список таблиц
                ...
                throw new JdbException("Empty db name [" + dbName + "] or tables list [" + tablesList + "]!");
            }
        }
    ...
    private static void loadSrsToDB(Connection conn, SerializableResultSet srs, String tableName) throws JdbException, SQLException {
        ...
        if (conn != null) {
            ...
            if ((srs != null) && (!srs.isEmpty())) {
                ...
                if (!StringUtils.isBlank(tableName)) {
                    checkRecord.append(tableName);
                } else {
                    checkRecord.append(srs.getTableName());
                }
                ...
                for (ArrayList<String> row : data) {
                    ...
                    // Запись с данным ключом не существует - формируем оператор INSERT (данные)
                        ...
                        for (int i = 1; i <= row.size(); i++) {
                            insertStatement.setObject(i, row.get(i - 1));
                        }
            ...
            // Получен пустой SRS
                ...
                throw new JdbException("Empty SerializableResultSet object instance!");
            }
        }
        // Соединение с СУБД пусто
        else {
            throw new JdbException("Empty DBMS connection!");
        }
    }
    ...
    private static void loadTableFromDisk(Connection conn, String path, String tableName,
        ...
        if (conn != null) {
                ...
                if (!StringUtils.isBlank(path) && new File(path).exists() && new File(path).isDirectory()) {
                    ...
                    if ((ObjectType.TABLE.equals(tableDescriptor.getObjectType()))
                        ...
                        if (!tableDescriptor.isEmpty()) {
                            ...
                            if (monitor != null) {
                                monitor.processMessage("[" + tableName + "] [0 / " + tableFiles.size() + "]");
                            }
                        ...
                        // Пустой дескриптор - нет списка объектов
                            ...
                            throw new JdbException("Empty table descriptor (no objects in list)!");
                        }
                    }
            ...
            // Указано пустое имя таблицы
                ...
                throw new JdbException("Empty table name!");
            }
        }
        // Соединение с СУБД пусто
        else {
        ...
        if (conn != null) {
            ...
            if (!StringUtils.isBlank(path) && new File(path).exists() && new File(path).isDirectory() && !MyIOUtils.isEmptyDir(path)) {
                        ...
                        if (!dbDescriptor.isEmpty()) {
                            ...
                            for (String table : tablesList) {
                                ...
                                if ((!StringUtils.isBlank(table) && dbDescriptor.containsItem(table))) {
                                    ...
                                    if (new File(tablePath).exists() && new File(tablePath).isDirectory() && !MyIOUtils.isEmptyDir(tablePath)) {
                                        ...
                                        // Перехват ИС для того, чтобы не оборвать загрузку БД.
                                            ...
                                            logger.error("Error loading table [" + table + "]! Message: " + e.getMessage());
                                    ...
                                    // обработка всей БД не прерывается.
                                        ...
                                        logger.error("Table [" + table + "] skipped! Reason: table path [" + tablePath +
                                ...
                                // обработка всей БД не прерывается.
                                    ...
                                    logger.error("Table [" + table + "] skipped! Reason: table name is empty or not present in db descriptor!");
                                }
                                ...
                                if (monitor != null) {
                                    monitor.processProgress((int) (progressTablesCounter * onePercent));
                                }
                            } // END OF FOR
                            // По окончании обработки таблиц установим значение прогресса в 100%
                            if (monitor != null) {
                                monitor.processProgress(100);
                            }
                        }
                        // Пустой дескриптор - в нем нет списка объектов
                        else {
                            throw new JdbException("Empty db descriptor (no objects in list)!");
                        }
                    }
                ...
                // Указанный список таблиц для загрузки пуст
                    ...
                    throw new JdbException("Empty tables list [" + tablesList + "]!");
                }
            }
            // Неверный путь к файлам таблицы
            else {
                throw new JdbException("Path [" + path + "] is empty, or not exists, or not a directory, or is empty directory!");
            }
        }
        // Соединение с СУБД пусто
        else {
    ...
    public static void main(String[] args) throws org.apache.commons.configuration2.ex.ConfigurationException, InstantiationException, IllegalAccessException {
        ...
        try {
            ...
            Connection supidConn = DBUtilities.getDBConn(supidConfig);
            Connection supidAppMSSQLConn = DBUtilities.getDBConn(supidAppMSSQLConfig);
