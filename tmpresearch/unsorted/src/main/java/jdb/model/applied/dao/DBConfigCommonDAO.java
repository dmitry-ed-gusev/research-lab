    ...
    public DBConfigCommonDAO(String loggerName, String configFile) throws org.apache.commons.configuration2.ex.ConfigurationException {
        ...
        if (DBConfigCommonDAO.config == null) {
            ...
            // Если указано пустое имя файла - нет инициализации, в лог - ошибка!
                ...
                logger.error("Config file name is EMPTY!");
            }
        }
        // Если класс конфига уже инициализирован - сообщим об этом
        else {
            logger.debug("DBConfig already initialized!");
        }
    }
    ...
    public DBConfigCommonDAO(String loggerName, DBConfig dbConfig) {
        ...
        if (DBConfigCommonDAO.config == null) {
            ...
            // Если указано пустое имя файла - нет инициализации, в лог - ошибка!
                ...
                logger.error("Received DBConfig is or has errors!");
            }
        }
        // Если класс конфига уже инициализирован - сообщим об этом
        else {
    ...
    public Connection getConnection() throws SQLException, IllegalAccessException, InstantiationException, ClassNotFoundException {
        ...
        if (config != null) {
            ...
            // Если же соединение пусто - ошибка
                ...
                throw new SQLException("Connection received from DBConfig is empty! Config file [" + configFile + "].");
            }
        }
        // Если же конфиг пуст - ошибка!
        else {
            throw new SQLException("DBConfig is null (maybe not initialized)!");
        }
    }
