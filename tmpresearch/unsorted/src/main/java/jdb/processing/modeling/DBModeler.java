...
public class DBModeler extends DBCommonProcessor {
    ...
    public DBModeler(DBConfig config) throws DBModuleConfigException {
        super(config);
    }
    ...
    private TreeSet<FieldStructureModel> getStructureFieldsList(ResultSet rs) throws SQLException, DBModelException {
        ...
        if ((rs != null) && (rs.next())) {
            ...
            do {
                ...
                if (rs.getInt(DBConsts.META_DATA_COLUMN_NULLABLE) == DatabaseMetaData.columnNoNulls) {
        ...
        if ((indexesRS != null) && (indexesRS.next())) {
            ...
            if (primaryKeysRS.next()) {
                ...
                do {
                    ...
                    if (!StringUtils.isBlank(keyColumn)) {
                        primaryKeysList.add(keyColumn.toUpperCase());
                    }
                }
            ...
            do {
                ...
                if (!StringUtils.isBlank(indexName) && !StringUtils.isBlank(fieldName)) {
                    ...
                    if ((primaryKeysList != null) && (primaryKeysList.contains(fieldName.toUpperCase()))) {
    ...
    public DBStructureModel getDBStructureModel(String dbName)
            throws SQLException, DBModuleConfigException, DBConnectionException, DBModelException, IllegalAccessException, InstantiationException, ClassNotFoundException {
        // Если конфигурация модуля содержит ошибки - возбуждаем ИС!
        String configErrors = null; // DBUtils.getConfigErrors(this.getConfig());
        ...
        if (!spider.isDBExists(localDbName)) {
            ...
            logger.debug("Database [" + localDbName + "] exists on current DBMS!");
        }
        ...
        if ((tablesList != null) && (!tablesList.isEmpty())) {
            ...
            try {
                ...
                connection = DBUtilities.getDBConn(this.getConfig());
                ...
                for (TableModel table : tablesList) {
                    ...
                    if ((table != null) && !StringUtils.isBlank(table.getTableName())) {
                        ...
                        try {
                            ...
                            if (!StringUtils.isBlank(table.getTableSchema())) {
                                schemaName = table.getTableSchema();
                            }
                    ...
                    // Если же таблица НУЛЛ или имя таблицы пусто - сообщаем в лог!
                        ...
                        logger.warn("NULL table in tables list or table name is empty!");
                    }
                } // END OF FOR CYCLE
        ...
        // Если же полученный список таблиц пуст - запишем в лог
            ...
            logger.warn("Tables list for this database [" + localDbName + "] is empty!");
        }
     ...
     * @throws DBModelException        ошибка при работе с моделью БД.
     */
    public DBStructureModel getDBStructureModel() throws SQLException, DBModuleConfigException, DBConnectionException, DBModelException, IllegalAccessException, ClassNotFoundException, InstantiationException {
        ...
        if ((tablesList != null) && (!tablesList.isEmpty())) {
            ...
            try {
                ...
                switch (this.getConfig().getDbType()) {
                    ...
                    case MSSQL_NATIVE:
                    case MYSQL:
                        selectDBQuery = "use " + localDbName;
                        break;
                    case INFORMIX:
                        selectDBQuery = "database " + localDbName;
                        break;
                }
                    ...
                    if ((table != null) && !StringUtils.isBlank(table.getTableName())) {
                        ...
                        try {
                            ...
                            if (rs.next()) {
                                ...
                                do {
                                    integrityTable.addKey(rs.getInt(DBConsts.FIELD_NAME_KEY));
                                } while (rs.next());
                            ...
                            // Сообщаем в лог, что ключи не нашли
                                ...
                                logger.debug("For table [" + fullTableName + "] keys list is not found!");
                            }
                        }
                ...
                if (!StringUtils.isBlank(this.getConfig().getDbName())) {
                    switch (this.getConfig().getDbType()) {
                        ...
                        case MSSQL_NATIVE:
                        case MYSQL:
                            returnDBQuery = "use " + this.getConfig().getDbName();
                            break;
                        case INFORMIX:
                            returnDBQuery = "database " + this.getConfig().getDbName();
                            break;
                    }
                }
    ...
    public DBIntegrityModel getDBIntegrityModel() throws SQLException, DBModuleConfigException, DBConnectionException, DBModelException, IllegalAccessException, ClassNotFoundException, InstantiationException {
    ...
    public DBTimedModel getDBTimedModel(String dbName)
            throws SQLException, DBConnectionException, DBModuleConfigException, DBModelException, IllegalAccessException, InstantiationException, ClassNotFoundException {
        // Если конфигурация модуля содержит ошибки - возбуждаем ИС!
        String configErrors = null; // DBUtils.getConfigErrors(this.getConfig());
        if (!StringUtils.isBlank(configErrors)) {
            throw new DBModuleConfigException(configErrors);
                    ...
                    if ((table != null) && !StringUtils.isBlank(table.getTableName())) {
                        ...
                        try {
                            ...
                            // Если нет данных - значит таблица просто пуста... Такая таблица не добавляется к результирующей модели.
                                ...
                                logger.warn("Table [" + fullTableName + "] doesn't have [" + DBConsts.FIELD_NAME_TIMESTAMP + "] field!");
                            }
                        } // END OF TRY
    ...
    public DBTimedModel getDBTimedModel()
            throws SQLException, DBConnectionException, DBModuleConfigException, DBModelException, IllegalAccessException, ClassNotFoundException, InstantiationException {
    ...
    public String getDifferenceReport(DBStructureModel current, DBStructureModel foreign) {
        ...
        if ((current != null) && (foreign != null)) {
            ...
            if ((current.getTables() != null) && (!current.getTables().isEmpty())) {
                ...
                report.append("table list is empty.");
            }
            report.append("\n");
            ...
            // Если при сравнении БД оказались равными - отчет не составляется
                ...
                report.append("DATABASES ARE EQUAL!");
            }
        }
        // Если переданный параметр пуст - скажем об этом в отчете
        else {
            report = new StringBuilder("Current and/or foreign database object is NULL!");
        }
