...
public class DBLoadCore {
    ...
    public static ArrayList<String> load(DBLoaderConfig config)
            throws SQLException, DBConnectionException, DBModuleConfigException, DBModelException, IOException, IllegalAccessException, InstantiationException, ClassNotFoundException {
        ...
        if (!new File(dbPath).exists()) {
            throw new IOException("Catalog [" + dbPath + "] doesn't exists!");
        } else if (!new File(dbPath).isDirectory()) {
            throw new IOException("Path [" + dbPath + "] not a catalog!");
        }
        // Сообщим о том, что каталог все проверки прошел
        else {
            logger.debug("Catalog [" + dbPath + "] is OK. Processing.");
        }
        ...
        if ((dbModel.getTables() != null) && (!dbModel.getTables().isEmpty())) {
            ...
            for (TableStructureModel table : dbModel.getTables()) {
                ...
                if (monitor != null) {
                    monitor.processMessage("[" + table.getTableName().toUpperCase() + "] [LOADING STARTED]");
                }
                ...
                if (config.getDbConfig().isTableAllowed(table.getTableName())) {
                    ...
                    if (tableDir.exists() && tableDir.isDirectory()) {
                        ...
                        try {
                            tableLoadErrors = TableLoadCore.load(tableLoaderConfig, table.getTableName());
                        ...
                        // Перехватываем возможные критические ИС при загрузке в БД одной таблицы
                            ...
                            logger.error(e.getMessage() + " TABLE [" + table.getTableName() + "].");
                ...
                // Если текущая таблица подпадает под ограничения - наффик!
                    ...
                    logger.warn("Current table [" + table.getTableName() + "] is deprecated or not allowed! Skipping.");
                }
        ...
        // Если же в полученной модели БД нет таблиц - сообщим об этом в лог (ничего не делаем)
            ...
            logger.error("Current database model is empty! Nothing to process.");
        }
