...
public class TableLoadCore {
    ...
    public static ArrayList<String> load(DBLoaderConfig config, String tableName)
        ...
        if (StringUtils.isBlank(tableName)) {
            ...
            logger.debug("Table name [" + tableName + "] is OK.");
        }
        ...
        if (!new File(tablePath).exists()) {
            throw new IOException("Catalog [" + tablePath + "] doesn't exists!");
        } else if (!new File(tablePath).isDirectory()) {
            throw new IOException("Path [" + tablePath + "] not a catalog!");
        } else if (!MyIOUtils.containFiles(tablePath)) {
            throw new IOException("Catalog [" + tablePath + "] doesn't contain files!");
        }
        // Сообщим о том, что каталог все проверки прошел
        else {
            logger.debug("Catalog [" + tablePath + "] is OK. Processing.");
        }
        // Локальная копия параметра "путь к сериализованной таблице" (сразу корректируем отсутствие символа / в конце пути)
        String localTablePath = MyIOUtils.fixFPath(tablePath, true);
        ...
        if (StringUtils.isBlank(config.getMonitorMsgPrefix())) {
            monitorMessagePrefix = "[" + tableName.toUpperCase() + "]";
        } else {
            monitorMessagePrefix = config.getMonitorMsgPrefix();
        }
        ...
        if (config.isTableAllowed(tableName)) {
            ...
            if ((files != null) && (files.length > 0)) {
                ...
                for (File file : files) {
                            ...
                            try {
                                ...
                                if ((sqlBatch != null) && (!sqlBatch.isEmpty())) {
                                    ...
                                    if (!useFullScript) {
                                        ...
                                        DBType dbType = config.getDbConfig().getDbType();
                                        logger.debug("-> " + dbType);
                                        if (config.isUseSetIdentityInsert() &&
                                                (DBType.MSSQL_JTDS.equals(dbType) || DBType.MSSQL_NATIVE.equals(dbType))) {
                                        ...
                                        // Если при обработке текущего батча данной таблицы не было ошибок - также сообщим об этом
                                            ...
                                            logger.debug("Executing part batch for table [" + tableName + "] finished without errors.");
                                        }
                                    }
                                    ...
                                    // Если же используем метод полного батча - формируем результирующий батч
                                        ...
                                        if (fullSqlBatch == null) {
                                            fullSqlBatch = new ArrayList<String>();
                                        }
                                ...
                                // Если же полученный батч пуст - сообщим в лог
                                    ...
                                    logger.warn("Batch for table [" + tableName + "] in file [" + (localTablePath + file.getName()) + "] is empty!");
                                }
                    ...
                    // Если текущий файл оказался не файлом (каталогом и т.п.), то не обрабатываем его
                        ...
                        logger.warn("Object [" + file.getAbsolutePath() + "] doesn't exists or not a file! Skipped!");
                    }
                ...
                if (useFullScript) {
                    ...
                    if ((fullSqlBatch != null) && (!fullSqlBatch.isEmpty())) {
                        ...
                        // Если при обработке данной таблицы не было ошибок - также сообщим об этом
                            ...
                            logger.debug("Executing full batch for table [" + tableName + "] finished without errors.");
                        }
                    }
                    // Если же полный sql-batch оказался пустым, ничего не делаем (просто сообщим в лог)
                    else {
                        logger.warn("Full sql-batch is empty! Nothing to execute!");
                    }
                }
            ...
            // Если полученный список файлов пуст
                ...
                logger.warn("File list is empty (no files in [" + localTablePath + "])!");
            }
        ...
        // Если текущая таблица подпадает под ограничения - наффик!
            ...
            logger.warn("Current table [" + tableName + "] is deprecated or not allowed! Skipping.");
        }
