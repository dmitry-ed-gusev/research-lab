...
public class BatchConfig extends CommonModuleConfig implements ConfigInterface {
    ...
    public BatchConfig(DBLoaderConfig config) {
        ...
        // Если указанный конфиг ошибочен - сообщаем в лог
            ...
            logger.error("BatchConfig() constructor: can't get data from DBLoaderConfig! Reason: " + configErrors);
        }
    }
    ...
    public void addSqlToBatchBody(String sql) {
        if (this.batchBody == null) {
            this.batchBody = new SimpleBatch(sql, this.isUseSqlFilter());
        } else {
            this.batchBody.addSqlToBatch(sql, this.isUseSqlFilter());
        }
    }
    ...
    public void addSqlToBatchRunOncePrefix(String sql) {
        if (this.batchRunOncePrefix == null) {
            this.batchRunOncePrefix = new SimpleBatch(sql, this.isUseSqlFilter());
        } else {
            this.batchRunOncePrefix.addSqlToBatch(sql, this.isUseSqlFilter());
        }
    }
    ...
    public void addSqlToBatchRunOncePostfix(String sql) {
        if (this.batchRunOncePostfix == null) {
            this.batchRunOncePostfix = new SimpleBatch(sql, this.isUseSqlFilter());
        } else {
            this.batchRunOncePostfix.addSqlToBatch(sql, this.isUseSqlFilter());
        }
    }
    ...
    public void addSqlToBatchPrefix(String sql) {
        if (this.batchPrefix == null) {
            this.batchPrefix = new SimpleBatch(sql, this.isUseSqlFilter());
        } else {
            this.batchPrefix.addSqlToBatch(sql, this.isUseSqlFilter());
        }
    }
    ...
    public void addSqlToBatchPostfix(String sql) {
        if (this.batchPostfix == null) {
            this.batchPostfix = new SimpleBatch(sql, this.isUseSqlFilter());
        } else {
            this.batchPostfix.addSqlToBatch(sql, this.isUseSqlFilter());
        }
    }
    ...
    public int getBatchSize() {
        int result = 0;
        if ((batchRunOncePrefix != null) && (!batchRunOncePrefix.isEmpty())) {
            result += batchRunOncePrefix.getBatchSize();
        }
        if ((batchRunOncePostfix != null) && (!batchRunOncePostfix.isEmpty())) {
            result += batchRunOncePostfix.getBatchSize();
        }
        if ((batchPrefix != null) && (!batchPrefix.isEmpty())) {
            result += batchPrefix.getBatchSize();
        }
        if ((batchPostfix != null) && (!batchPostfix.isEmpty())) {
            result += batchPostfix.getBatchSize();
        }
        if ((batchBody != null) && (!batchBody.isEmpty())) {
            result += batchBody.getBatchSize();
        }
        return result;
    }
    ...
    public String getConfigErrors() {
        ...
        else if (this.getBatchSize() <= 0) {
            result = "All parts of SQL batch is empty!";
        }
        return result;
    }
