...
public class FileSystem implements Serializable {
     ...
     */
    private static final char DEFAULT_DRIVE_LETTER_DELIMITER = ':';
        ...
        if ((homeDir != null) && (!homeDir.trim().equals(""))) {
            ...
            if ((!new File(dir).exists()) || (!new File(dir).isDirectory()))
                ...
                if ((deprecatedDirsList != null) && (deprecatedDirsList.length > 0)) {
                    ...
                    if ((this.deprecatedDirsList == null) || (this.deprecatedDirsList.isEmpty()))
                        this.deprecatedDirsList = new ArrayList<String>();
                    ...
                    for (String deprecatedDir : deprecatedDirsList) {
                        logger.debug("Adding deprecated catalog: [" + deprecatedDir + "].");
                        this.deprecatedDirsList.add(deprecatedDir);
                    }
     ...
     * @throws IOException ИС возникает, если корневой каталог не существует на диске.
     */
    public FileSystem(String homeDir) throws IOException {
        this(homeDir, null);
    }
    ...
    private void addFile(String filePath) throws IOException {
        ...
        if ((path != null) && (new File(path).exists()) && (new File(path).isFile()) &&
            ...
            file.setCrcCode(MyIOUtils.getChecksum(path));
            ...
            logger.warn("addFile: file path [" + path + "] is not valid or exists in deprecated list or not a file! Can't add file!");
        }
    }
    ...
    private void scanFileSystem(String path) {
        ...
        // Перехват ИС
            ...
            logger.error("Error while scanning filesystem: " + e.getMessage());
        }
    }
    ...
    public void addDeprecatedDir(String dir) {
        ...
        if ((dir != null) && (!dir.trim().equals("")) /*&& (this.homeDir.indexOf(dir) == -1)*/) {
            ...
            if (this.deprecatedDirsList == null) {
                this.deprecatedDirsList = new ArrayList<String>();
            }
    ...
    private boolean containsDeprecated(String path) {
        ...
        if ((this.deprecatedDirsList != null) && (!this.deprecatedDirsList.isEmpty()) && (path != null) && (!path.trim().equals(""))) {
            ...
            catList.addAll(Arrays.asList(localPath.split(String.valueOf(DEFAULT_DIR_DELIMITER))));
            ...
            for (String deprecatedDir : deprecatedDirsList) {
                ...
                if (deprecatedDir.indexOf(String.valueOf(DEFAULT_DIR_DELIMITER)) > -1) {
                    ...
                    if (deprecatedDir.indexOf(String.valueOf(DEFAULT_DRIVE_LETTER_DELIMITER)) > -1) {
                        if (deprecatedDir.endsWith(String.valueOf(DEFAULT_DIR_DELIMITER))) {
                            localDeprecated.append(deprecatedDir);
                        } else {
                            localDeprecated.append(deprecatedDir).append(DEFAULT_DIR_DELIMITER);
                        }
                    }
                    ...
                    // "/" в начало и в конец пути
                        ...
                        if (!deprecatedDir.startsWith(String.valueOf(DEFAULT_DIR_DELIMITER))) {
                            localDeprecated.append(DEFAULT_DIR_DELIMITER);
                        }
                        localDeprecated.append(deprecatedDir);
        ...
        if (result) {
            logger.warn("Path [" + path + "] contains deprecated. SKIPPED!");
        }
        //logger.debug("FILESYSTEM [containsDeprecated]: path [" + path + "] contains deprecated -> " + result);
        return result;
    }

    public ArrayList<String> getDeprecatedDirsList() {
        return this.deprecatedDirsList;
    }

    public void setDeprecatedDirsList(ArrayList<String> deprecatedDirsList) {
        this.deprecatedDirsList = deprecatedDirsList;
    }
     ...
     */
    public ArrayList<DiskFile> getFilesList() {
        ...
        */
                ...
                for (DiskFile foreignFile : foreignFS.filesList) {
                    if (!this.isFileExists(foreignFile)) {
                        ...
                        if (!filesList.contains(foreignFile.getFileName())) {
                            filesList.add(foreignFile.getFileName());
                        }
                    }
                }
