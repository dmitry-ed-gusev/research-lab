...
public class DBSpider extends DBCommonProcessor {
     ...
     */
    public DBSpider(DBConfig config) throws DBModuleConfigException {
        super(config);
    }
     ...
     */
    public ArrayList<TypeMapping> getTypesMappings() throws SQLException, DBConnectionException, DBModuleConfigException, IllegalAccessException, InstantiationException, ClassNotFoundException {
        ...
        //    throw new DBModuleConfigException(dbConfigErrors);
        //}
        ...
        try {
            ...
            connection = DBUtilities.getDBConn(getConfig());
    ...
    public ArrayList<TableModel> getUserTablesList(String dbName, DBTableType[] tableTypes)
        ...
        if (!StringUtils.isBlank(dbName)) {
            ...
            localDbName = this.getConfig().getDbName();
        } else {
        ...
        if (!this.isDBExists(localDbName)) {
            throw new SQLException("Database [" + localDbName + "] doesn't exists on current DBMS!");
        } else {
            logger.debug("Database [" + localDbName + "] exists on current DBMS! Processing.");
        }
        ...
        if ((tableTypes != null) && (tableTypes.length > 0)) {
            ...
            for (int i = 0; i < tableTypes.length; i++) {
                localTableTypes[i] = tableTypes[i].getStrValue();
            }
        } else {
            localTableTypes = null;
        }
        ...
        try {
                ...
                do {
                    ...
                    if (!StringUtils.isBlank(tableName)) {
                        ...
                        if (DBType.INFORMIX.equals(this.getConfig().getDbType())) {
                            if (DBConsts.SYSCATALOG_INFORMIX.contains(tableName.toUpperCase())) {
                                isTableSystem = true;
                            }
                        }
                        ...
                        // Если же таблица оказалась системной - просто сообщим об этом
                            ...
                            logger.debug("Table [" + tableName + "] is system table in DBMS [" + this.getConfig().getDbType().getStrValue() + "].");
                        }
                    }
                    // Если полученное имя таблицы пусто - обломс! :)
                    else {
                        logger.warn("Empty table name!");
                    }
                }
                while (tablesRS.next());
            }
            // Если же список таблиц пуст - сообщим об этом в лог
                ...
                logger.warn("Received tables list is empty or NULL!");
            }
    ...
    public ArrayList<TableModel> getUserTablesList(String dbName)
            throws SQLException, DBConnectionException, DBModuleConfigException, IllegalAccessException, ClassNotFoundException, InstantiationException {
    ...
    public ArrayList<String> getUserTablesPlainList(String dbName, DBTableType[] tableTypes, boolean useSchemaPrefix)
            throws DBConnectionException, DBModuleConfigException, SQLException, IllegalAccessException, ClassNotFoundException, InstantiationException {
    ...
    public ArrayList<String> getUserTablesPlainList(String dbName, boolean useSchemaPrefix)
            throws DBConnectionException, DBModuleConfigException, SQLException, IllegalAccessException, InstantiationException, ClassNotFoundException {
     ...
     * @throws DBConnectionException   ошибки соединения с СУБД.
     */
    public ArrayList<String> getDBSList() throws SQLException, DBConnectionException, DBModuleConfigException, IllegalAccessException, InstantiationException, ClassNotFoundException {
        ...
        if (
            ...
            try {
                ...
                if (basesRS.next()) {
                    ...
                    logger.warn("Cannot create list of bases for current DBMS server [meta data is empty]! ");
                }
            }
            // Закроем соединение с СУБД и окрытые курсоры
            finally {
        ...
        // Если тип СУБД Derby или Dbf - возвращаем пустой список
            ...
            logger.warn("DBMS type is Derby or Dbf - list of databases is empty!");
        }
     ...
     * @throws DBConnectionException   ошибки соединения с СУБД.
     */
    public boolean isDBExists(String dbName) throws SQLException, DBConnectionException, DBModuleConfigException, IllegalAccessException, ClassNotFoundException, InstantiationException {
        ...
        if (!StringUtils.isBlank(dbName)) {
            ...
            if (
                ...
                if ((dbsList != null) && (!dbsList.isEmpty()) && (dbsList.contains(dbName.toUpperCase()))) {
                    result = true;
                } else {
                    logger.warn("Databases list is empty or not contains DB [" + dbName + "].");
                }
            }
            ...
            // может быть и пустым - это значит, что в БД просто нет таблиц.
                ...
                if (new File(dbName).exists() && new File(dbName).isDirectory()) {
                    result = true;
                } else {
                    logger.warn("Catalog [" + dbName + "] doesn't exists.");
                }
            }
        } else {
            logger.warn("DB name is EMPTY! Can't check existence.");
        }
        return result;
    }
     ...
     * @throws DBConnectionException   ошибки соединения с СУБД.
     */
    public ArrayList<String> getTablesList(boolean ignoreConstraints) throws SQLException, DBConnectionException, DBModuleConfigException, IllegalAccessException, InstantiationException, ClassNotFoundException {
        ...
        if (StringUtils.isBlank(this.getConfig().getDbName())) {
            throw new DBModuleConfigException("Name of DB is empty!");
        }
        // Результирующий список таблиц
        ArrayList<String> list = null;
        Connection connection = null;
        ...
        try {
            ...
            // Если же список таблиц пуст - сообщим об этом в лог
                ...
                logger.warn("Received tables list is empty!");
            }
        }
        // Закроем соединение с СУБД и окрытые курсоры
        finally {
        ...
        if (!StringUtils.isBlank(tableName)) {
            ...
            if ((tablesList != null) && (!tablesList.isEmpty())) {
                ...
                for (String table : tablesList) {
                    if (table.equals(tableName.toUpperCase())) {
                        result = true;
                    }
                }
            }
            // Если же полученный список таблиц пуст - сообщим в лог
                ...
                logger.warn("Tables list is empty!");
            }
        } else {
            logger.warn("Empty table name!");
        }
     ...
     * @throws DBConnectionException   ошибки соединения с СУБД.
     */
    public boolean isAbsTableExists(String tableName) throws SQLException, DBConnectionException, DBModuleConfigException, IllegalAccessException, InstantiationException, ClassNotFoundException {
