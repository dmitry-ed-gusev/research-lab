...
public class SqlBatchBuilder {
    ...
    public static ArrayList<String> getBatchFromTableFile(DBConfig config, String fullFilePath, boolean deleteSource,
            ...
            throws IOException, SQLException, DBConnectionException, DBModuleConfigException, DBModelException, IllegalAccessException, InstantiationException, ClassNotFoundException {
        ...
        //if (!DBUtils.isConnectionValid(config)) {
        ...
        if (StringUtils.isBlank(fullFilePath)) {
            ...
            throw new IOException("File [" + fullFilePath + "] doesn't exists!");
        } else if (!new File(fullFilePath).isFile()) {
            throw new IOException("Path [" + fullFilePath + "] not a file!");
        } else {
            logger.debug("File [" + fullFilePath + "] is OK. Deserializing (reading) object.");
        }
        // Проверка указанного имени таблицы (не пусто ли оно)
        if (StringUtils.isBlank(tableName)) {
            throw new SQLException("Specifyied table name is empty!");
        } else {
            logger.debug("Table name is OK.");
        }
        ...
        if (StringUtils.isBlank(currentTableName)) {
            throw new DBModelException("Table name in deserialized object TableDTOModel is EMPTY!");
        }
        ...
        if (config.isTableAllowed(currentTableName)) {
            ...
            Connection connection = DBUtilities.getDBConn(config);
            ...
            for (RowDTOModel rowModel : tableDTOModel.getRows()) {
                ...
                if (keyFieldModel != null) {
                    ...
                    try {
                        id = Integer.parseInt(keyFieldModel.getFieldValue());
                    } catch (NumberFormatException e) {
                        logger.error("Can't parse KEY value! [" + e.getMessage() + "]. Record was skipped!");
                        id = 0;
                    }
                    ...
                    if (id > 0) {
                        ...
                        // Если же запрос пуст - сообщим в лог
                            ...
                            logger.warn("Generated data update/insert query is empty!");
                        }
                    }
                    // Ключевое поле в данной строке не имеет значения (значение получить не удалось)
                    else {
                        logger.error("Can't receive KEY field [" + DBConsts.FIELD_NAME_KEY + "] value.");
                    }

                }
                // Ключевое поле не найдено в обрабатываемой строке
                else {
                    logger.warn("KEY field in rowModel was not found!");
                }
            } // END OF FOR CYCLE
        ...
        // Если же обработка таблицы не разрешена - сообщим об ошибке
            ...
            logger.warn("Current table [" + currentTableName + "] is deprecated or not allowed! Skipping.");
        }
