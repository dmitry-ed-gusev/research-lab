...
class PSqlStatement {
  ...
  */
  private final static String LOGGER_LOG_FORMAT_DEFAULT = "%d{dd/MM/yyyy HH:mm:ss}(%-5rms) {%17M} [%t] %p %c %x - %m%n";


 /**
     * Определяем формат ведение журнала (на основе шаблона).
     */
    private static PatternLayout patternLayout = new PatternLayout(LOGGER_LOG_FORMAT_DEFAULT);
    /**
