...
public class DBEngineer {
    ...
    public ArrayList<String> makeIntegrity(DBIntegrityModel db, DBProcessingMonitor monitor)
            throws DBConnectionException, SQLException, DBModuleConfigException, DBModelException, IllegalAccessException, ClassNotFoundException, InstantiationException {
        // Если еще не инициализировано поле DBIntegrityChecker - инициализируем его
        if (integrityChecker == null) {
            integrityChecker = new DBIntegrityChecker(config);
        }
    ...
    public ArrayList<String> makeIntegrity(DBIntegrityModel db)
            throws DBConnectionException, SQLException, DBModuleConfigException, DBModelException, IllegalAccessException, InstantiationException, ClassNotFoundException {
        // Если еще не инициализировано поле DBIntegrityChecker - инициализируем его
        if (integrityChecker == null) {
            integrityChecker = new DBIntegrityChecker(config);
        }
     ...
     */
    public String testDBMSConnection() throws IllegalAccessException, InstantiationException, ClassNotFoundException {
        String result = null;
        Connection connection = null;
        try {
            connection = DBUtilities.getDBConn(this.config);
            // Если полученный объект Connection пуст - соединение мы не установили (почему - ХЗ?)
            if (connection == null) {
                throw new SQLException("Can't establish a connection! Unknown reason!");
            }
        }
        // Перехват ИС
    ...
    public boolean isAbsTableExists(String tableName)
            throws DBConnectionException, SQLException, DBModuleConfigException, IllegalAccessException, ClassNotFoundException, InstantiationException {
        // Если еще не инициализировано поле DBSpider - инициализируем его
        if (spider == null) {
            spider = new DBSpider(config);
        }
