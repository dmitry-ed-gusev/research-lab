...
public final class JdbcUrlHelper {
    ...
    public static String getInformixJdbcUrl(DBConfig config) {
        ...
        if (StringUtils.isBlank(configErrors)) {
            ...
            if (!StringUtils.isBlank(config.getDbName())) {
                jdbcUrl.append("/").append(config.getDbName());
            }
            ...
            if (!StringUtils.isBlank(config.getConnParams())) {
                if (config.getConnParams().startsWith(";")) {
                    jdbcUrl.append(config.getConnParams());
                } else {
                    jdbcUrl.append(";").append(config.getConnParams());
                }
            }
        }
        // Если конфиг пуст - сообщим об этом
        else {
            logger.error("There are config errors [" + configErrors + "].");
        }
        // Формирование конечного результата
        if (jdbcUrl != null) {
            result = jdbcUrl.toString();
        } else {
            result = null;
        }
    ...
    public static String getMysqlJdbcUrl(DBConfig config) {
        ...
        if (StringUtils.isBlank(configErrors)) {
            ...
            if (!StringUtils.isBlank(config.getDbName())) {
                jdbcUrl.append(config.getDbName());
            }
    ...
    public static String getMssqlNativeJdbcUrl(DBConfig config) {
        ...
        if (StringUtils.isBlank(configErrors)) {
            ...
            // Если имя БД указано - подключаемся к ней, если же не указано - подключаемся к серверу в целом
            if (!StringUtils.isBlank(config.getDbName())) {
                jdbcUrl.append(";database=").append(config.getDbName());
            }
