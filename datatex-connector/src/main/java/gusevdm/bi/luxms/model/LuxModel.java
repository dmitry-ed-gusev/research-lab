...
public class LuxModel {
     ...
     *  These identificators are used for map keys, identificators for periods are autogenerated, starting from 1.
     */
    ...
    public static Map<Long, LuxPeriod> generatePeriods(String... years) throws ParseException {
        LOGGER.debug(String.format("LuxPeriod.generatePeriods() is working. Years: %s", Arrays.toString(years)));
        ...
        long quarterId;
        long monthId;
        // temporary parent identificators
        long yearParentId;
        long quarterParentId;
        // autoincremented ID counter
        int idCounter = 1;
        ...
        for (String year : years) { // <- YEARS
            ...
            periods.put(yearId, new LuxPeriod(idCounter, year, -1, startDate, LuxPeriodType.YEAR));
            ...
            for (String quarter : quartersNames) { // <- QUARTERS
                ...
                periods.put(quarterId, new LuxPeriod(idCounter, title, yearParentId, startDate, LuxPeriodType.QUARTER));
                ...
                for (int monthCounter = 3 * (quarterCounter - 1); monthCounter < 3 * quarterCounter; monthCounter++) { // <- MONTHS
                    ...
                    startDate = LUX_DATE_FORMAT.parse(year + "-" +
                            ((monthCounter + 1) > 9 ? (monthCounter + 1) : "0" + (monthCounter + 1)) + "-01");
                    // create period object (period ID - auto generated, ID of period in a map - as described in doc above)
                    period    = new LuxPeriod(idCounter, title, quarterParentId, startDate, LuxPeriodType.MONTH);
                    idCounter++;
    ...
    //    System.out.println("-> " + LuxModel.generatePeriods("2013", "2014"));
    //}
}
